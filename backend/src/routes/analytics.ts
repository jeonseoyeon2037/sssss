...
// GET /api/analytics/personal-tasks - personal_tasks 컬렉션의 모든 데이터 가져오기
router.get('/personal-tasks', async (_req, res) => {
  try {
...
// GET /api/department-tasks - department_tasks 컬렉션의 모든 데이터 가져오기
router.get('/department-tasks', async (_req, res) => {
  try {
    const db = getFirestoreDB();
    const snapshot = await db.collection('department_tasks').get();
